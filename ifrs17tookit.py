...
def render_bf_calculator():
    ...
    # =============================================================================
#  BORNHUETTER-FERGUSON (BF) CALCULATOR — COMPLETE
# =============================================================================

def render_bf_calculator():
    """Bornhuetter-Ferguson IBNR Calculator with Inflation & Discounting."""
    show_breadcrumb()
    st.markdown('<div class="hero"><h1>Bornhuetter-Ferguson (BF) IBNR Calculator</h1><p>A-priori ELR combined with development pattern. Upload claims and premium data. Set ELR per group. Optional inflation adjustment and discounting.</p></div>', unsafe_allow_html=True)
    st.markdown('<div class="main-container">', unsafe_allow_html=True)

    col1, col2 = st.columns(2)
    with col1: client_name = st.text_input("Client Name", value="Client", key="bf_cn").strip()
    with col2: pass

    st.markdown('<div class="section-container"><h3>IBNR Period & Grain</h3></div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: from_date = st.date_input("From Date", value=date(2021,1,1), key="bf_fd")
    with c2: to_date = st.date_input("To Date", value=date(2025,12,31), key="bf_td")
    ...
    # Claims file
    st.markdown('<div class="section-container"><h3>Upload Claims Data</h3></div>', unsafe_allow_html=True)
    claims_file = st.file_uploader("Choose claims file", type=["csv","xlsx","xls"], key="bf_cf")
    ...
    if claims_file is not None:
        try:
            ...
            all_cols = df.columns.tolist()
            c1, c2 = st.columns(2)
            with c1: loss_col = st.selectbox("Loss Date column", [""]+all_cols, key="bf_lc")
            with c2: report_col = st.selectbox("Report Date column", [""]+all_cols, key="bf_rc")
            if not loss_col or not report_col: st.warning("Select both date columns."); st.stop()
            
            remaining = [c for c in all_cols if c not in [loss_col, report_col]]
            grouping_cols = st.multiselect("Grouping columns (optional)", remaining, key="bf_gc")
            
            num_cands = [c for c in df.columns if c not in [loss_col,report_col]+grouping_cols and pd.api.types.is_numeric_dtype(df[c])]
            if not num_cands: st.error("No numeric columns."); st.stop()
            amount_cols = st.multiselect("Claim Amount columns", num_cands, key="bf_ac")
            if not amount_cols: st.warning("Select at least one amount column."); st.stop()
            ...
            # ELR per Group
            st.markdown("---"); st.markdown('<div class="section-container"><h3>Set ELR per Group</h3></div>', unsafe_allow_html=True)
            elr_dict = {}
            ec = st.columns(min(3, len(unique_groups)))
            for i, g in enumerate(unique_groups):
                with ec[i%3]: elr_dict[g] = st.number_input(f"ELR for {g} (%)", 0.0, 200.0, 70.0, 1.0, key=f"bf_elr_{g}")/100
            
            elr_summary = pd.DataFrame({"Group": list(elr_dict.keys()), "ELR": [f"{v*100:.1f}%" for v in elr_dict.values()]})
            st.dataframe(elr_summary, use_container_width=True)

            # Premium file
            st.markdown('<div class="section-container"><h3>Upload Premium Data</h3><p>Must have one row per accident period matching the grouping columns.</p></div>', unsafe_allow_html=True)
            st.markdown(f"**Required:** {n_periods} rows. Columns: {', '.join(unique_groups)}")
            premium_file = st.file_uploader("Choose premium file", type=["csv","xlsx","xls"], key="bf_pf")
            ...
            if premium_file is not None:
                ...
                for i, g in enumerate(unique_groups):
                    with pc[i%3]:
                        sc = st.selectbox(f"Premium for {g}", prem_cols, key=f"bf_pm_{g}")
                        if len(prem_df) == n_periods:
                            premiums_dict[g] = pd.to_numeric(prem_df[sc], errors='coerce').fillna(0).tolist()
                
                if len(prem_df) != n_periods: st.error(f"Premium file has {len(prem_df)} rows, need {n_periods}")
                elif all(len(v)>0 for v in premiums_dict.values()): st.success("Premium data validated.")

            # Options
            st.markdown("---"); st.markdown("### Options")
            c1, c2 = st.columns(2)
            with c1:
                use_discounting = st.checkbox("Apply Discounting", key="bf_ud")
                spot_rates = None; flat_rate = None
                if use_discounting:
                    use_curve = st.checkbox("Use Yield Curve", key="bf_uc")
                    if use_curve:
                        yc_file = st.file_uploader("Yield Curve file", type=["csv","xlsx","xls"], key="bf_yc")
                        if yc_file is not None:
                            yc_ext = yc_file.name.split('.')[-1].lower()
                            yc_df = pd.read_csv(yc_file) if yc_ext=='csv' else pd.read_excel(yc_file)
                            yc_df = yc_df.dropna()
                            if len(yc_df.columns)>=2:
                                ...
                                spot_rates = np.clip(f_int(pm), 0, 1.0)
                    else: flat_rate = st.number_input("Annual discount rate (%)", 0.0, 50.0, 5.0, key="bf_fr")/100
            with c2:
                use_inflation = st.checkbox("Apply Inflation Adjustment", key="bf_ui")
                cum_inflation = None; per_period_rates = None
                if use_inflation:
                    inf_file = st.file_uploader("Inflation file", type=["csv","xlsx","xls"], key="bf_if")
                    if inf_file is not None:
                        inf_ext = inf_file.name.split('.')[-1].lower()
                        inf_df = pd.read_csv(inf_file) if inf_ext=='csv' else pd.read_excel(inf_file)
                        inf_df = inf_df.dropna()
            ...
            # ---- RUN BF ----
            if st.button("Run BF Calculation", key="bf_run", use_container_width=True) and all(len(v)>0 for v in premiums_dict.values()):
                if grouping_cols: groups = df_filtered[grouping_cols].drop_duplicates().to_dict("records")
                else: groups = [{"__all__":"All Data"}]
                ...
                for grp in groups:
                    ...
                    for ac in amount_cols:
                        ...
                        group_key = glabel.split(" | ")[0] if " | " in glabel else glabel
                        gelr = elr_dict.get(group_key, elr_dict.get("All Data", 0.7))
                        prems = premiums_dict.get(group_key, premiums_dict.get("All Data", [0]*n_periods))
                        ...
                        for i in range(n_ay):
                            ...
                            if last_obs<len(pct_unpaid):
                                bf_ibnr = eu*pct_unpaid[last_obs]; pdv = (1-pct_unpaid[last_obs])*100
                            else: bf_ibnr=0; pdv=100
                            rows.append({"AP":i,"Current":current,"Premium":prems[i],"ELR":gelr,"Expected":eu,"PctDev":pdv,"BF_IBNR":bf_ibnr})
                        
                        res_df = pd.DataFrame(rows)
                        res_df["BF_IBNR_Nominal"] = res_df["BF_IBNR"]
                        ...
                        if use_inflation and cum_inflation is not None and per_period_rates is not None:
                            ...
                            for i in completed.index:
                                ...
                                for idp,dp in enumerate(dc):
                                    ...
                                    total+=ir*fci(i+dp)
                                nm[i]=total
                            res_df["BF_IBNR_Nominal"] = res_df["AP"].map(lambda ap: nm.get(ap,0.0))
                        
                        nom_total = res_df["BF_IBNR_Nominal"].sum()
                        st.metric(f"Total BF IBNR", f"{nom_total:,.2f}")
                        disp = res_df.copy()
                        for c in ["Current","Premium","Expected","BF_IBNR","BF_IBNR_Nominal"]:
                            if c in disp.columns: disp[c] = disp[c].apply(lambda x: f"{x:,.2f}")
                        st.dataframe(disp, use_container_width=True)
                        
                        all_summary.append({"Group":glabel,"Amount":ac,"ELR":gelr,"BF_IBNR_Nominal":nom_total})
                ...
                if all_summary:
                    ...
                    output = BytesIO()
                    with pd.ExcelWriter(output, engine='openpyxl') as w:
                        pd.DataFrame(all_summary).to_excel(w, index=False, sheet_name='BF_Summary')
                    output.seek(0)
                    sc = re.sub(r'[\\/*?:"<>|]',"",client_name).strip() or "Client"
                    st.download_button("Download Excel", data=output, file_name=f"{sc}_BF_IBNR_Results.xlsx", key="bf_dl", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
